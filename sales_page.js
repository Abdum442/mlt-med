...
makeSalesPage(mainContainer, modalContainer);


const orderList = [];

function makeSalesPage(container, modal) {
  
  const salesPage = makeGrid(medicinalProducts, customerDetails, modal);
  container.appendChild(salesPage);
}



function makeGrid(productData, customerData, modal) {
  ...
  const productCard = makeProductCard(productData);
  const customerCard = makeCustomerCard(customerData);
...
function productInfo(productData){
  ...
  nameInput.addEventListener('input', manageProductInfo);
  ...
  productData.sort(compareExpiry).forEach(item => {
    const option = document.createElement('option');
    option.text = item.name;
    option.value = item.name;
    option.setAttribute('data-id', item.id);
    option.setAttribute('data-stock', item.stockLevel);
    option.setAttribute('data-expiryDate', item.expiryDate);
  ...
  stockInput.setAttribute('list', 'sales-stock-list');
  stockInput.setAttribute('placeholder', 'Choose..');
  stockInput.id = 'sales-stock-level';
  stockInput.name = 'sales-stock-level';

  const stockDataList = document.createElement('datalist');
  stockDataList.name = 'sales-stock-level';
  stockDataList.id = 'sales-stock-list';

  col25.appendChild(stockInput);
  col25.appendChild(stockDataList);
  ...
  submitBtn.addEventListener('click', orderTable);
...
function orderTable(){
  ...
  const itemStock = stockLevelInput.value;
  ...
  for(let i = 0; i < medicinalProducts.length; i++) {
    if (medicinalProducts[i].name === itemName && medicinalProducts[i].stockLevel === itemStock) {
      selectedItem['expiryDate'] = medicinalProducts[i].expiryDate;
      selectedItem['iD'] = medicinalProducts[i].id;
      selectedItem['price'] = medicinalProducts[i].price;
      selectedItem['total'] = Number(medicinalProducts[i].price) * Number(quantity) + '';
      break;
    }
  ...
  gradTotalCell.innerHTML = formatNumber(totalPrice);

  
  for (let i = 0; i < medicinalProducts.length; i++) {
    if (selectedItem.iD === medicinalProducts[i].id) {
      let currentLevel = Number(medicinalProducts[i].stockLevel);
      let quant = Number(selectedItem.quantity);
      let stockLevel = currentLevel - quant;
      if ( stockLevel >= 0 ) {
        medicinalProducts[i].stockLevel = '' + stockLevel;
        const data = [selectedItem.iD, selectedItem.name, 
                      selectedItem.expiryDate, selectedItem.quantity,
                      formatNumber(Number(selectedItem.price)), formatNumber(Number(selectedItem.total))];
        ...
        orderList.pop();
        alert(`Please, enter a lower quantity. The maximum available stock is ${medicinalProducts[i].stockLevel}.`)
      }
    }
  }
}
...
function printSalesReport(){
  ...
  hrow[hrow.length - 1].className = 'sales-text-right';
  let Gtotal = 0;
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i].getElementsByTagName('td');
    row[row.length - 1].className = 'sales-text-right';
    Gtotal += reformatNumber(row[row.length-1].innerText);
  }
...
function manageProductInfo () {
  ...
  for(let i = 0; i < medicinalProducts.length; i++) {
    if(medicinalProducts[i].name === selectedProductName && Number(medicinalProducts[i].stockLevel) > 0) {
      productStocks.push(medicinalProducts[i].stockLevel);
    }
  }
  if (productStocks.length < 1) {
    productStocks.push('Out of Stock');
  }
  stockLevelInput.value = '';
  stockLevelDataList.innerHTML = '';
  productStocks.forEach(stock => {
    ...
    stockLevelDataList.appendChild(option);
  })
}

function manageTabs(tabId) {
  const currentOrder = document.getElementById('sales-current-order');
  const buttonCard = document.getElementById('sales-button-card');
  const tabBtn = document.getElementById(tabId);
  tabBtn.style.backgroundColor = '#ddd';
  if (tabId === 'sales-order-tab' ) {
    document.getElementById('sales-order-tab').style.backgroundColor = '#ddd';
    document.getElementById('sales-hold-tab').style.backgroundColor = '';
    currentOrder.style.display = 'block';
    buttonCard.style.display = 'block';
    
  } else if (tabId === 'sales-hold-tab' ) {
    document.getElementById('sales-order-tab').style.backgroundColor = '';
    document.getElementById('sales-hold-tab').style.backgroundColor = '#ddd';
...
// medicinalProducts.sort(compareExpiry);

// console.log(medicinalProducts);